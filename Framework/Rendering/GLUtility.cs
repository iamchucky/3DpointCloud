...
namespace Magic.Rendering
{
    ...
    public class GLCameraFree : GLCamera
    {
      ...
      double aspect;
			...
			public Vector3 Location { get { return new Vector3(location.x, location.y, location.z); } }
    ...
    public class GLCameraOrtho : GLCamera
    {
      ...
      public override void ApplyProjection()
      {
		...
		public static void DrawPointList(GLPen pen, List<Vector3> pts)
		{
			DrawPointList(pen, pts, false);
		}
		public static void DrawPointList(GLPen pen, List<Vector3> pts, bool zEqualsZero)
		{
			Color color = pen.color;
			Gl.glBegin(Gl.GL_POINTS);
			foreach (Vector3 vp in pts)
			{
				...
				Gl.glVertex3d(vp.X, vp.Y, zEqualsZero?0:vp.Z);
			}
			Gl.glEnd();
		}
		...
		public static void Draw3DRectangle(Color color, Vector3 bl, Vector3 br, Vector3 tl, Vector3 tr)
		{
			//pen.GLApplyPen();
    ...
    public static void DrawLine3D(GLPen pen, float aX, float aY, float aZ, float bX, float bY, float bZ)
    {
      ...
      Gl.glEnd();
		...
		public static void DrawPoint(GLPen pen, Vector2 p)
		{
    ...
    public static void DrawPoint3D(GLPen pen, Vector3 p)
    {
      ...
      Gl.glEnd();
		...
		public static void DrawWireframeBox3D_glVertexOnly(Vector3 a, Vector3 b)
    {
      Gl.glVertex3d(a.X, a.Y, a.Z);
    ...
    public static void DrawCross3D(GLPen pen, Vector3 p, float size)
    {
      ...
      Gl.glEnd();
		...
		public static void DrawCross(GLPen pen, Vector2 p, float size)
		{
			Vector2 p1 = new Vector2(p.X, p.Y + size / 2.0f);
			Vector2 p2 = new Vector2(p.X, p.Y - size / 2.0f);
			Vector2 p3 = new Vector2(p.X + size / 2.0f, p.Y);
			Vector2 p4 = new Vector2(p.X - size / 2.0f, p.Y);
			DrawLine(pen, p1, p2);
			DrawLine(pen, p3, p4);
		}

		public static void DrawDiamond(GLPen pen, Vector2 p, float size)
    {
			Vector2 p1 = new Vector2(p.X, p.Y + size / 2.0f);
			Vector2 p2 = new Vector2(p.X, p.Y - size / 2.0f);
			Vector2 p3 = new Vector2(p.X + size / 2.0f, p.Y);
      ...
      DrawLine(pen, p4, p1);
        ...
        public static void DrawLineLoop(GLPen pen, Vector2[] points)
        {
            pen.GLApplyPen();
            Gl.glBegin(Gl.GL_LINE_LOOP);
            foreach (Vector2 p in points)
		...
		{
			FillRectangle(color, rect, 1.0f);
		}

		public static void FillRectangle(Color color, RectangleF rect, float alpha)
		{
    ...
    public static void GoToTransformYPR(float Yrad, float Prad, float Rrad)
    {
      ...
      Gl.glRotatef((Rrad * 180 / (float)Math.PI), 1.0f, 0.0f, 0.0f);
		...
		public static Color FalseColor(float value, float min, float max)
		{
			if (float.IsNaN(value)) value = 0;
			float range = max - min;
			return FalseColor(value / range - min / range);
		}
		public static Color FalseColor(float v)
		{
			if (v >= 1.0f) return Color.FromArgb(255, 0, 0);
			if (v <= 0.0f) return Color.FromArgb(0, 0, 255);

			if (v <= .5f)
			{
				float b = (1.0f - v * 2.0f) * 255.0f;
				float g = 255.0f - b;
	...
	/*	public static void DrawCar(PointF rearAxle, float h, int txid)
		...
		*/
		...
		public static void DrawCar(PointF rearAxle, float h, Color color, string label, bool rotate90, Color labelColor, float width, float centerpointToFront, float height)
		{
			float carWidth = width;
			float carIMUtoFront = centerpointToFront;
			float carHeight = height;
			//the rear axle point is just the center of the car in x and the rear of the car in y			
      ...
      if (label != "") GLUtility.DrawString(label, new Font("verdana", 1.0f), labelColor, new PointF(+.20f, 0));
			GLUtility.DrawCross(new GLPen (Color.Pink,1.0f), new Vector2(0, 0), r.Width / 2);
			GLUtility.ComeBackFromVehicleCoordinates();
		}
    ...
    public static void DrawGrid3D(int gridStep, WorldTransform w, float z, Color lineColor, Color textColor, Color radiiColor)
    {
      ...
      GLUtility.EnableNiceLines();
		...
		internal static void GoToSensorPose(Magic.Common.SensorPose laserToBody)
		{
			GLUtility.GoToTransformXYZ((float)laserToBody.x, (float)laserToBody.y, (float)laserToBody.z);
			GLUtility.GoToTransformYPR((float)laserToBody.yaw, (float)laserToBody.pitch, (float)laserToBody.roll);			
		}

		internal static void ComeBackFromSensorPose()
		{
			GLUtility.ComeBackFromTransform();
			GLUtility.ComeBackFromTransform();
		}

		internal static void DrawLine(GLPen pen, PointF p1, PointF p2)
		{
			DrawLine(pen, Vector2.FromPointF(p1), Vector2.FromPointF(p2));
		}

		internal static void DrawCircle(GLPen circPen, Vector2 vector2, float p)
		{
			DrawCircle(circPen, vector2.ToPointF(), p);
		}
	}
