...
DWORD Sync1394Camera::CamThread ()
{
	...
	{	
		...
		{
			if (fcount %100==0)	printf("COULD NOT AQUIRE AN IMAGE FROM THE CAMERA %d.\n", camId);			
		}
		...
		if ((config.AGC) && (config.isSlave == false))
		{
			if (fcount%3 == 0)
			{
				...
				if (fcount %100==0)
				{
					printf("CAM%d AGC: Median: %d Brightness: %d Gain: %d Shutter: %d Error: %f\n",camId,median,newbright,newgain,newShutter,error);
				}
			}
...
bool Sync1394Camera::InitCamera(int cameraID, SyncCamParams m_config) 
{
	...
	if (config.isSlave)	
	{ 
		if (DEBUG_SYNC1394) printf("Using SLAVE configuration.\n");
	}
	...
	if (!config.AutoGain)
	{
		...
		{	
			printf("I have autogain, cannot use manual gain.");
		}
	...
	if (!config.AutoShutter)
	{
		...
		if (ctrl->StatusAutoMode() == true)
		{	
			printf("I have autoshutter, cannot use manual mode");
		}
	}
	else
	{
	...
	if (!config.AutoBrightness)
	{
		...
		if (ctrl->StatusAutoMode() == true)
		{	
			printf("I have autobrightness, cannot use manual mode");
		}
