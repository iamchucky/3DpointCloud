...
namespace Magic.Common.Path
{
	...
	public class PointPath : IPath
	{
		...
		public PointPath(IList<Waypoint> initialPoints)
			: this()
		{
			if (initialPoints.Count == 0) return; //Can't create a path with no points
			else if (initialPoints.Count < 2)
			{
				segments = new List<IPathSegment>(1); //Creates a path with one segment whose start and endpoint is the same
				segments.Add(new LinePathSegment(initialPoints[0].Coordinate, initialPoints[0].Coordinate));
			}
			else
			{
				segments = new List<IPathSegment>(initialPoints.Count - 1);
				for (int i = 0; i < initialPoints.Count - 1; i++)
				{
					segments.Add(new LinePathSegment(initialPoints[i].Coordinate, initialPoints[i + 1].Coordinate));
				}
			}
		}

		public PointPath(IList <Vector2> initialPoints) : this()
		{
			if (initialPoints.Count == 0) return; //Can't create a path with no points
			else if (initialPoints.Count < 2)
			{
				segments = new List<IPathSegment>(1); //Creates a path with one segment whose start and endpoint is the same
				segments.Add(new LinePathSegment(initialPoints[0], initialPoints[0]));
			}
			else
			{
				segments = new List<IPathSegment>(initialPoints.Count - 1);
				for (int i = 0; i < initialPoints.Count - 1; i++)
				{
					segments.Add(new LinePathSegment(initialPoints[i], initialPoints[i + 1]));
				}
			}
		}
		...
		public PointPath(IEnumerable<IPathSegment> initial)
		{
		...
		public IPath Clone()
		{
			return new PointPath(CopyEnumerator(), coordMode);
		}
			...
			else
			{
				while (dist < 0)
				{
					...
					if (dist < 0)
					{
						...
						{
							// otherwise, move the PointOnPath to the end of the previous segment
		...
		{
			segments.RemoveRange(index, count);
		}
        ...
        {
            return "";
            //return segments[0].Start.ToString() + " -> " + segments[segments.Count - 1].End.ToString();
        }
