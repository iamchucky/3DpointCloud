...
namespace Magic.Common.Path {
	...
	public class CirclePathSegment : IPathSegment {
		...
		public CirclePathSegment(Vector2 center, double radius, double startAngle, double endAngle, bool ccw) {
		...
		public Vector2 Start {
			get {
				return new Vector2(center.X + radius*Math.Cos(startAngle), center.Y + radius*Math.Sin(startAngle));
			}
			set {
		...
		public Vector2 End {
			get {
				return new Vector2(center.X + radius*Math.Cos(endAngle), center.Y + radius*Math.Sin(endAngle)); 
			}
			set {
				throw new NotSupportedException();
			}
		...
		public double DistanceToGo(Vector2 pt) {
		...
		public PointOnPath ClosestPoint(Vector2 pt) {
			Vector2 ptOnCircle = center + (pt - center).Normalize(radius); 
		...
		public void Transform(Magic.Common.Mapack.Matrix3 m) {
			throw new NotSupportedException();
		}
