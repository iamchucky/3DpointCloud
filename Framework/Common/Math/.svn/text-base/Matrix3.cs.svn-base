...
namespace Magic.Common.Mapack {
	[Serializable]
	public unsafe struct Matrix3 : IVector2Transformer {
		...
		{
			return new Matrix(this[0, 0], this[1, 0], this[2, 0], this[0, 1], this[1, 1], this[2, 1], this[0, 2], this[1, 2], this[2, 2]);
		}

		public void GetEuler (out double ez, out double ey, out double ex)
		{
			//Compute the EULER angles zyx from this global to body dcm
			
			ey = Math.Asin(-this[0, 2]);

			if (Math.Abs (this[0, 2]) <= 1) 	// cos(y) != 0 Gimbal-Lock
			{
				ex = Math.Atan2(this[1, 2], this[2, 2]);
				ez = Math.Atan2(this[0, 1], this[0, 0]);
			}
			else
			{
				ex = 0;
				ez = Math.Atan2(-this[1, 0], this[1, 1]);
			}
		}
		...
		#region IVector2Transformer Members
		...
		public Vector2[] TransformPoints(Vector2[] c) {
			if (c == null)
				return null;

			Vector2[] ret = new Vector2[c.Length];
		...
		public Vector2[] TransformPoints(ICollection<Vector2> c) {
			if (c == null)
				return null;

			Vector2[] ret = new Vector2[c.Count];
			int i = 0;
			foreach (Vector2 pt in c) {
		...
		public void TransformPointsInPlace(Vector2[] c) {
			for (int i = 0; i < c.Length; i++) {
				c[i] = TransformPoint(c[i]);
			}
		}

		public void TransformPointsInPlace(IList<Vector2> c) {
