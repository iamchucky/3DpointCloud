...
namespace Magic.Common.Mapack {
	...
	public class Matrix : ILoggable {
		...
		public Matrix(double m00, double m01, double m02,
									...
									double m20, double m21, double m22) : this(3,3)
		{			
			data[0][0] = m00;
			data[0][1] = m01;
			data[0][2] = m02;
        ...
        /// <summary>Concatenate matrix along specified dimension.</summary>
        /// <param name="dim">dim = 1 for [A; B]. dim = 2 for [A, B].</param>
        /// <param name="m">Matrix B</param>
        public Matrix Concat(int dim, Matrix m)
        {
            int row = this.rows;
            ...
            if (dim == 1)
            {
                row = row + m.rows;
                if (col != m.columns)
                    throw new ArgumentException("Matrix columns must agree.");
            }
            else if (dim == 2)
            {
                col = col + m.columns;
                if (row != m.rows)
                    throw new ArgumentException("Matrix rows must agree.");
            }
            else
                throw new ArgumentException("Argument out of range.");
		...
		public void SetSubMatrix(int startRow, int endRow, int startColumn, int endColumn, Matrix subMatrix)
		{
			if ((startRow > endRow) || (startColumn > endColumn) || (startRow < 0) || (startRow >= this.rows) || (endRow < 0) || (endRow >= this.rows) || (startColumn < 0) || (startColumn >= this.columns) || (endColumn < 0) || (endColumn >= this.columns))
			{
				throw new ArgumentException("Argument out of range.");
			}
			if (subMatrix.columns != (endColumn - startColumn + 1) || subMatrix.rows != (endRow - startRow + 1))
				throw new ArgumentException("row or/and column range does not match with subMatrix");
			for (int i = startColumn; i <= endColumn; i++)
			{
				for (int j = startRow; j <= endRow; j++)
				{
					this[j, i] = subMatrix[j - startRow, i - startColumn];
				}
			}
		}
		...
		public static Matrix Multiply(double left, Matrix right)
		{
		...
		public static Matrix GenerateEllipse(int numPoint, double centerX, double centerY, Matrix sMatrix, int numSigma)
		{
		...
		public string ToLog()
		{
			//This is logged in the following format:
			//numRows	numCols	data(row0,col0)	data(row0,col1)....data(row0,colNumCols)	data(row1,col0).... .... data(rowNumRows,colNumCols)
			...
			sb.Remove(sb.Length - 1, 1);//remove the last tab character
			return sb.ToString();
		}
